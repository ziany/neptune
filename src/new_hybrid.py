...
positive=ngramGenerator.mostFreqList('../data/used/positive_processed.csv',UNIGRAM_SIZE) # add as needed 
negative=ngramGenerator.mostFreqList('../data/used/negative_processed.csv',UNIGRAM_SIZE)
neutral=ngramGenerator.mostFreqList('../data/used/neutral_processed.csv',UNIGRAM_SIZE)
...
def modiKNN(k,z): # aggregate negative and positive scores for knn
    ...
    l.append(z[k]+z[k+1])
    l.extend(z[k+2:-3])
    l.append((z[len(z)-3]+z[len(z)-2]))
    l.append(z[len(z)-1])
    return l

def modiSVM(k,z): # remove neutral polarity score 
    l=z[0:k]
    l.extend(z[k+1:-1]) # remove neutral score 
...
def testFile(filename,knn_model,svm_model): # function to load test file in the csv format : sentiment,tweet 
    ...
    print " mispos = %f, misneg = %f, misneu = %f" % (mispos,misneg,misneu)
    print nbr
...
for C in [0.01*i for i in range(1,11)]:
    for K in ['poly','linear']:
        for N in range(5,15):
            a=validateHybrid(X,Y,N,K,C)
            print "C = %f, N = %f, K = %s : ACC = %f " % (C,N,K,a)
            if (a > ACC):
                ACC=a
                N_NEIGHBORS=N
                KERNEL_FUNCTION=K
...
print 'best n : '+str(N_NEIGHBORS)
print 'best c : '+str(C_PARAMETER)
print 'best k : '+str(KERNEL_FUNCTION)
